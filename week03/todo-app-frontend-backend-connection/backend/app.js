...
app
    ...
    .post(async (req, res) => {
       try {
            ...
            const lastTask = await Task.findOne().sort('-id');
...
app.
    ...
    .patch(async (req, res) => {
        ...
        console.log('receive updates: ', req.body);
        try {
    ...
    .delete(async (req, res) => {
        try {
            ...
            res.status(200).json({ message: 'Succesfully deleted the task: ', task: deletedTask });
        } catch(err) {
            res.status(400).json({ error: err.message });
        }
    });
